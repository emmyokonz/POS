 ...
 */
class Auth1407201901 extends MY_AuthController
{
	public $data = [];
	...
	public function login()
	{
		...
		$this->template->title($this->lang->line('login_heading'));
		// validate form input
		...
		if ($this->form_validation->run() === TRUE)
		{
			...
			{
				log_activity($this->session->userdata('user_id'),'Logged in');
			...
			else
			{
				...
				$this->redirect('login', 'refresh'); // use redirects instead of loading views for compatibility with MY_Controller libraries
			}
		}
		else
		{
	...
	{
		$this->template->title($this->lang->line('forgot_password_heading'));
	...
	public function reset_password($code = NULL)
	{
		...
		if ($user)
		{
			...
			if ($this->form_validation->run() === FALSE)
			{
				...
				if(validation_errors() || $this->session->flashdata('message'))
				{
					$this->template->build("error",(validation_errors()) ? validation_errors() : $this->session->flashdata('message'));
				}
				...
				// render
				$this->template->build('auth/reset_password', $this->data);
			}
			else
			{
